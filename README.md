# 🛠️ querycanvas - Effortlessly Manage Your Databases

## 📥 Download Now
[![Download](https://img.shields.io/badge/Download-QueryCanvas-brightgreen)](https://github.com/bhrissss/querycanvas/releases)

## 🎉 Introduction
Welcome to **querycanvas**, the AI-powered database client designed to enhance your experience with Cursor and VS Code. This application comes with smart SQL formatting and customizable result displays, making database management intuitive and straightforward.

## 🚀 Getting Started
1. **System Requirements**
   - Operating System: Windows, macOS, or Linux
   - Visual Studio Code: Version 1.50 or higher
   - Basic understanding of SQL (no programming knowledge required)

2. **Supported Databases**
   - MySQL
   - PostgreSQL
   - SQLite
   
## 📦 How to Download & Install
To get started, you need to download the application from our releases page. Follow these steps:

1. Click the download button below to visit the release page:
   [Download QueryCanvas](https://github.com/bhrissss/querycanvas/releases)

2. On the Releases page, you will see a list of available versions. Choose the latest version to ensure you have the most recent features and fixes.

3. Locate the installation file suitable for your operating system:
   - For Windows, look for a `.exe` file.
   - For macOS, look for a `.dmg` file.
   - For Linux, look for a `.tar.gz` file.

4. Click on the appropriate file to start the download.

5. Once the download is complete, locate the file on your computer.

6. Open the file to run the installation process. Follow the on-screen instructions to complete the installation.

7. After installation, open VS Code and look for the QueryCanvas extension in your installed extensions section.

8. Configure your database connections and start exploring the features.

## ⚙️ Features
- **Smart SQL Formatting:** Write SQL queries with ease. The AI helps format your code, saving you time and reducing errors.
  
- **Customizable Result Display:** Tailor the presentation of your query results. Choose how you want your data displayed for better clarity.

- **User-Friendly Interface:** Designed to be straightforward, even for users with no technical background.

- **Cross-Platform Support:** Use the application on Windows, macOS, or Linux.

## 🔧 Usage Instructions
After installing QueryCanvas, you can connect to your databases. Here’s how:

1. Open QueryCanvas in VS Code.
2. Click on the “New Connection” button.
3. Fill in the required fields for your database:
   - Hostname
   - Username
   - Password
   - Database Name
4. Click “Connect” to establish the connection.

## 🖥️ Support and Troubleshooting
If you encounter issues, here are some tips:

- **Check Your Connection Settings:** Ensure that all details are correct.
  
- **Consult the Help Section:** Access the built-in help section in the application for guidance.

- **Visit Our Community Page:** Join discussions or ask for help from other users. You can find the community page linked on the main application screen.

## 📌 Additional Resources
For more detailed information about features and usage, please visit the following links:

- [Documentation](https://github.com/bhrissss/querycanvas/wiki)
- [FAQ](https://github.com/bhrissss/querycanvas/wiki/FAQ)

## 💬 Feedback
We value your opinions! Please share your feedback or feature requests on our Issues page.

## 📜 License
This project is licensed under the MIT License. See the [LICENSE](https://github.com/bhrissss/querycanvas/blob/main/LICENSE) file for more details. 

Thank you for choosing QueryCanvas. Enjoy a smoother database experience!